...
string read_init_constant_facts_or_next_marker(istream &in) {
    // 中文说明：search 从 preprocess 输出中读取静态 init 常量，并返回
    // 下一段 marker。返回 marker 是为了兼容旧格式：旧文件在
    // end_global_constraint 后会直接出现 begin_SG。
